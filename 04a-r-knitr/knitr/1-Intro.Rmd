...
## How is a R Markdown Document (.Rmd) Created?

```{r, echo = FALSE}
knitr::include_graphics("knitr.png")
```

- Takes the .Rmd file converts to a pandoc file (.md) and then can otuput to multiple formats (.html, .pdf)
- What we will be focusing on 
- [Picture Source](https://bookdown.org/yihui/rmarkdown-cookbook/rmarkdown-process.html)
...
- knitr: use install.packages("rmarkdown") to install the knitr package in R
...
- Can you make knitr show only the plot, without code, messages, errors, etc.?
...
+ [More Options](https://bookdown.org/yihui/rmarkdown-cookbook/kable.html)

+ kableExtra Package - extension of basic functionality of kable function [More Info](https://bookdown.org/yihui/rmarkdown-cookbook/kableextra.html)
...
+ Note: Working directory is the directory where your .Rmd file is saved. 
+ Use 'out.height="50%" & out.width="50%"' to play with figure size.
...
``![](IncludeGraphic.png)`` $\\$
``![](IncludeGraphic.png){width=50%}`` - Reduce width
...
- Create a table of a dataframe in your R markdown file. Play with the different styling options of the table. (Can try and read in your own data to create a table!)

---

## Other things that you can do!

+ If have a pdf output, can enter "\newpage" into the markdown part of document to create a new page

+ In RStudio, you can click the menu Edit -> Check Spelling to spell-check an Rmd document
  - There is real time spell-check in newer versions of RStudio, but useful if checking whole document at once. 
  
- Create slide shows with R markdown
  + [Website for more Info](https://rmarkdown.rstudio.com/lesson-11.html)
  + [Built in Markdown Presentations](https://bookdown.org/yihui/rmarkdown/presentations.html)
