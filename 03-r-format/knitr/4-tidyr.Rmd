...
- Let's rerun with the following in `guess_max` function
```{r echo=TRUE}
...
```{r, echo=TRUE, fig.width=7, fig.height=3, purl = FALSE}
flights$date <- ymd(paste(flights$year, flights$month, flights$day, sep = "-"))
delay.dat <- flights %>% 
    group_by(date) %>% 
    summarise(dep_delay = mean(dep_delay, na.rm = TRUE))
...
```{r, echo=TRUE, fig.width=6.5, fig.height=3.5, warning=FALSE, purl=FALSE}
long_billboard <- pivot_longer(billboard, cols = X1:X76,
    names_to = "week",
    values_to = "rank", values_drop_na = TRUE) %>%
  mutate(week = parse_number(week))
