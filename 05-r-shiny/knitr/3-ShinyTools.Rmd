...
+ This output is given the name `hist`

+ `Outputs` on the ui.R side work with `render` functions on the server.R side.
...
<br>
See [shiny cheatsheet](https://shiny.rstudio.com/images/shiny-cheatsheet.pdf) for render and output options.
...
+ To reference the input from the ui, use `input$inputID`.
...
+ Hint: see [conditionalPanel](https://shiny.rstudio.com/reference/shiny/1.6.0/conditionalPanel.html)
...
- Ideas: 
    ...
    - Use `filter()` in `dplyr` and `checkboxInput()` to plot user-selected subsets
